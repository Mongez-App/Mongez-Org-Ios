...
public struct TeamCardView: View {
    let team: Team
    let onClick: () -> Void
    
    public init(team: Team, onClick: @escaping () -> Void = {}) {
        self.team = team
    ...
    private var placeholderColor: Color {
        ...
        let index = abs(team.id.hashValue) % colors.count
        return colors[index]
    }
    ...
    private var eventChips: [String] {
        ...
        for event in team.events {
            counts[event.lowercased(), default: 0] += 1
        }
                    ...
                    GeometryReader { geometry in
                        ZStack(alignment: .leading) {
                            ...
                            RoundedRectangle(cornerRadius: 4)
                                ...
                                .frame(width: geometry.size.width * (CGFloat(team.progress) / 100.0), height: 6)
                        }
                    }
                    .frame(height: 6)
                }
